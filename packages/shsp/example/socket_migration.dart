...
///
/// [ShspSocketMigratable] and [DualShspSocketMigratable] allow the underlying
...
// ── Example 1: Single-socket migration with ShspSocketMigratable ─────────────

Future<void> singleSocketMigration() async {
  print('\n=== Example 1: ShspSocketMigratable migration ===');

  final original = await ShspSocket.bind(InternetAddress.anyIPv4, 0);
  final wrapper = ShspSocketMigratable(original);
...
// ── Example 3: constructor with pre-configured migratables ────────────────

Future<void> preConfiguredMigratableExample() async {
  print('\n=== Example 3: DualShspSocketMigratable(ipv4Migratable: ...) ===');

  final ipv4 = await ShspSocket.bind(InternetAddress.anyIPv4, 0);
  final ipv4Migratable = ShspSocketMigratable(ipv4);

  // Pre-configure the migratable before injecting it
  ipv4Migratable.setListeningCallback(() => print('  [pre-configured] listening'));

  final migratable = DualShspSocketMigratable(ipv4Migratable: ipv4Migratable);
  ...
  print('  ipv4SocketImpl is ShspSocketMigratable: '
      '${migratable.ipv4SocketImpl is ShspSocketMigratable}');
...
Future<void> main() async {
  ...
  try {
    ...
    await preConfiguredMigratableExample();
