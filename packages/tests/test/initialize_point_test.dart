...
void main() {
  group('initializePointShsp Tests', () {
    tearDown(() {
    ...
    test('successfully creates and registers dual socket in DI', () async {
      ...
      expect(dualSocket, isNotNull, reason: 'DualShspSocket should be registered in DI');
      expect(dualSocket, isA<DualShspSocket>(), reason: 'Should be DualShspSocket');
    });
    ...
    test('registered socket is open and usable', () async {
      ...
      expect(dualSocket.localPort, greaterThan(0), reason: 'Should have valid port');
    });
    ...
    test('dual socket contains both IPv4 and IPv6 support', () async {
      ...
      final dualSocket = SingletonDIAccess.get<IDualShspSocket>();
      expect(dualSocket, isA<DualShspSocket>(), reason: 'Should support dual stack');
    });
    ...
    test('socket has valid local address', () async {
      ...
      final dualSocket = SingletonDIAccess.get<IDualShspSocket>();
      expect(dualSocket.localAddress, isNotNull, reason: 'Should have local address');
    });
    ...
    test('socket has compression codec', () async {
      ...
      final dualSocket = SingletonDIAccess.get<IDualShspSocket>();
      expect(dualSocket.compressionCodec, isNotNull,
    ...
    test('registry singleton is registered in DI', () async {
      ...
      expect(registry, isNotNull, reason: 'RegistrySingletonShspSocket should be registered');
      expect(registry, isA<RegistrySingletonShspSocket>(),
    ...
    test('socket can set and retrieve message callbacks', () async {
      ...
      final dualSocket = SingletonDIAccess.get<IDualShspSocket>();
      final peerInfo = PeerInfo(address: InternetAddress.loopbackIPv4, port: 8888);
    ...
    test('socket can send data to peer', () async {
      ...
      final dualSocket = SingletonDIAccess.get<IDualShspSocket>();
      final peerInfo = PeerInfo(address: InternetAddress.loopbackIPv4, port: 9999);
    ...
    test('socket profile can be extracted', () async {
      ...
      expect(profile, isA<ShspSocketProfile>(), reason: 'Should be ShspSocketProfile');
    });
    ...
    test('can retrieve socket multiple times from DI (same instance)', () async {
      await initializePointShsp();
    ...
    test('socket implements all required interfaces', () async {
      ...
      expect(dualSocket, isA<IShspSocket>(), reason: 'Should implement IShspSocket');
      expect(dualSocket, isA<IDualShspSocket>(), reason: 'Should implement IDualShspSocket');
    });
    ...
    test('socket has IPv4 socket accessible', () async {
      ...
      expect(typedSocket.ipv4Socket, isNotNull, reason: 'IPv4 socket should exist');
      expect(typedSocket.ipv4Socket.isClosed, isFalse, reason: 'IPv4 socket should be open');
    });
    ...
    test('socket has IPv6 socket if available', () async {
      ...
      if (hasIPv6) {
        expect(typedSocket.ipv6Socket, isNotNull,
            reason: 'IPv6 socket should exist if available');
        if (typedSocket.ipv6Socket != null) {
          expect(typedSocket.ipv6Socket!.isClosed, isFalse,
              reason: 'IPv6 socket should be open');
        }
      }
    });

    test('multiple peers can register callbacks on initialized socket', () async {
      await initializePointShsp();

      final dualSocket = SingletonDIAccess.get<IDualShspSocket>();

      final peer1 = PeerInfo(address: InternetAddress.loopbackIPv4, port: 7001);
      final peer2 = PeerInfo(address: InternetAddress.loopbackIPv4, port: 7002);
      final peer3 = PeerInfo(address: InternetAddress.loopbackIPv4, port: 7003);
    ...
    test('socket state remains consistent after operations', () async {
      ...
      for (int i = 0; i < 10; i++) {
        final peer = PeerInfo(address: InternetAddress.loopbackIPv4, port: 6000 + i);
        dualSocket.setMessageCallback(peer, (_) {});
      }

      expect(dualSocket.isClosed, isFalse, reason: 'Socket should still be open');
      expect(dualSocket.localPort, greaterThan(0), reason: 'Port should be valid');
    });
    ...
    test('socket local address is IPV4', () async {
      ...
      expect(address?.type, equals(InternetAddressType.IPv4),
    ...
    test('socket raw datagram socket is accessible', () async {
      ...
      expect(rawSocket, isA<RawDatagramSocket>(),
    ...
    test('initialization is repeatable - new sockets each time', () async {
      ...
      expect(port2, greaterThan(0), reason: 'New socket should have valid port');
