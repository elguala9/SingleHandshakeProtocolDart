...
void main() {
  group('ShspSocketSingleton', () {
    ...
    test('bind returns a singleton instance', () async {
      ...
      final s1 = await ShspSocketSingleton.bind(info: info, callbacks: callbacks);
      final s2 = await ShspSocketSingleton.bind(info: info, callbacks: callbacks);
    ...
    test('can use as ShspSocket', () async {
      final info = ShspSocketInfoSingleton();
      final callbacks = MessageCallbackMapSingleton();
      final singleton = await ShspSocketSingleton.bind(info: info, callbacks: callbacks);
