...
void main() {
  group('Factories', () {
    ...
    test('ShspPeerFactory create from inputs', () async {
      final peerInfo =
          PeerInfo(address: InternetAddress.loopbackIPv4, port: 12345);
      final rawA =
          await RawDatagramSocket.bind(InternetAddress.loopbackIPv4, 0);
      sockets.add(rawA);
      final peer = ShspPeerFactory.create(
          remotePeer: peerInfo,
          socket: ShspSocketFactory.createFromSocket(rawA));
    ...
    test('ShspInstanceFactory create from inputs', () async {
      ...
      final instance = ShspInstanceFactory.createFromSocket(
          remotePeer: peerInfo, rawSocket: raw1, keepAliveSeconds: 5);
      expect(instance, isNotNull);
      final raw2 =
          await RawDatagramSocket.bind(InternetAddress.loopbackIPv4, 0);
      sockets.add(raw2);
      final instanceInput = ShspInstanceInput(
          remotePeer: peerInfo, rawSocket: raw2, keepAliveSeconds: 5);
