...
class RawShspSocket implements RawDatagramSocket {
  ...
  @override
  int send(List<int> buffer, InternetAddress address, int port) => socket.send(buffer, address, port);
  ...
  @override
  void leaveMulticast(InternetAddress group, [NetworkInterface? interface]) => socket.leaveMulticast(group, interface);
  ...
  StreamSubscription<RawSocketEvent> listen(void Function(RawSocketEvent event)? onData, {Function? onError, void Function()? onDone, bool? cancelOnError}) =>
    socket.listen(onData, onError: onError, onDone: onDone, cancelOnError: cancelOnError);
  ...
  @override
  Stream<RawSocketEvent> where(bool Function(RawSocketEvent) test) => socket.where(test);

  @override
  Stream<S> map<S>(S Function(RawSocketEvent) convert) => socket.map(convert);

  @override
  Stream<E> asyncMap<E>(FutureOr<E> Function(RawSocketEvent) convert) => socket.asyncMap(convert);

  @override
  Stream<E> asyncExpand<E>(Stream<E>? Function(RawSocketEvent) convert) => socket.asyncExpand(convert);

  @override
  Stream<S> expand<S>(Iterable<S> Function(RawSocketEvent) convert) => socket.expand(convert);

  @override
  Stream<RawSocketEvent> handleError(Function onError, {bool Function(dynamic)? test}) => socket.handleError(onError, test: test);

  @override
  Stream<S> transform<S>(StreamTransformer<RawSocketEvent, S> streamTransformer) => socket.transform(streamTransformer);

  @override
  Future<RawSocketEvent> reduce(RawSocketEvent Function(RawSocketEvent, RawSocketEvent) combine) => socket.reduce(combine);
  ...
  @override
  Stream<RawSocketEvent> takeWhile(bool Function(RawSocketEvent) test) => socket.takeWhile(test);

  @override
  Stream<RawSocketEvent> skip(int count) => socket.skip(count);

  @override
  Stream<RawSocketEvent> skipWhile(bool Function(RawSocketEvent) test) => socket.skipWhile(test);

  @override
  Stream<RawSocketEvent> distinct([bool Function(RawSocketEvent, RawSocketEvent)? equals]) => socket.distinct(equals);
  ...
  @override
  Future<RawSocketEvent> firstWhere(bool Function(RawSocketEvent) test, {RawSocketEvent Function()? orElse}) => socket.firstWhere(test, orElse: orElse);

  @override
  Future<RawSocketEvent> lastWhere(bool Function(RawSocketEvent) test, {RawSocketEvent Function()? orElse}) => socket.lastWhere(test, orElse: orElse);

  @override
  Future<RawSocketEvent> singleWhere(bool Function(RawSocketEvent) test, {RawSocketEvent Function()? orElse}) => socket.singleWhere(test, orElse: orElse);

  @override
  Future<RawSocketEvent> elementAt(int index) => socket.elementAt(index);

  @override
  Stream<RawSocketEvent> timeout(Duration timeLimit, {void Function(EventSink<RawSocketEvent>)? onTimeout}) => socket.timeout(timeLimit, onTimeout: onTimeout);

  @override
  Future<bool> contains(Object? needle) => socket.contains(needle);

  @override
  Future<void> forEach(void Function(RawSocketEvent) action) => socket.forEach(action);

  @override
  Stream<R> cast<R>() => socket.cast<R>();

  @override
  Future<void> pipe(StreamConsumer<RawSocketEvent> streamConsumer) => socket.pipe(streamConsumer);
}
